...
namespace iOPS_ODataV4.Controllers.OdataV4
{
    ...
    */
    public class GSAlarmHistoriesController : ODataController
    {
        ...
        {
            return SingleResult.Create(db.GSAlarmHistories.Where(gSAlarmHistory => gSAlarmHistory.Id == key));
        }


        // GET: odata/GSAlarmHistories(5)/Tag
        [EnableQuery]
        public SingleResult<Tag> GetTag([FromODataUri] long key)
        {
            return SingleResult.Create(db.GSAlarmHistories.Where(m => m.Id == key).Select(m => m.Tag));
        }
